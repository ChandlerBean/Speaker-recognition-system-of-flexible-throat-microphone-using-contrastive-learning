#为什么num_classes是1000，因为这个网络原本是用于1000类的图像分类的
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, in_chs=3, num_classes=20,
        ...
        #self.num_features = last_num * block.expansion
        self.fc = nn.Sequential(
            #nn.Linear(256, 64),
            nn.Linear(512, num_classes) #self.num_features改64
            #nn.Softmax()
        )
...
def resnet4(pretrained=False, **kwargs):
    ...
    #model.load_state_dict(torch.load(r"F:\NewDataset20220316\model\teacher\CL2_100_all_160_M25_model.pkl"))

    model.fc = nn.Sequential(
        nn.Linear(512,64),
        nn.ReLU(),
        nn.Linear(64, 20), #self.num_features改64
        nn.Softmax()
    )
    return model
...
def resnet18(pretrained=False, **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(BasicBlock, [2, 1, 1, 1], in_chs=3, **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['resnet18']))
    #model.fc = torch.nn.Linear(64, last_num) # 读取权重成功后，把最后一层从（512，1000）修改为（512，20）
    model.fc = nn.Sequential(
        nn.Linear(512,64),
        nn.ReLU(),
        nn.Linear(64, 20), #self.num_features改64
